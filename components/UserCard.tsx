...
interface UserCardProps {
  ...
  imageUrl?: string;
  groupChatId: string;
  handleOpenMessageBox?: (id: string) => Promise<void>; // Thêm thuộc tính này và đ   ánh dấu là có thể undefined
}
    ...
    >
      <div className="flex flex-row space-x-3 items-center w-full pr-2 pl-2">
        ...
        <div className="flex flex-col justify-start">
          ...
          <p className="text-[14px] font-extralight text-gray-500">
            {lastMessage}
          </p>
        </div>
      </div>
      <div className="absolute top-2 right-3">
        <span className="text-xs text-gray-500">{lastTimeRelative}</span>
      </div>
    </div>
  );
};
